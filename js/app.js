...
const HEIGHT_RANGES = {
  ...
  mini: [
    { id:'base', label:'77cm ~ 230cm 미만',  desc:'추가 키트 불필요', needKit:false, icon:'fa-check-circle' },
    { id:'kit',  label:'230cm 이상',           desc:'추가 키트 필요',   needKit:true,  icon:'fa-plus-circle'  }
  ]
};
...
const INSTALL_CRITERIA = {
  ...
  minHeightMini: 77,      maxHeightMini: 230,       // 미니: 77~230cm
...
function resolveYoutubeKeys() {
  ...
  const kitThresh = s.productType === 'mini' ? 230 : 148;
...
const App = {
  ...
  validateStep2() {
    ...
    if (!isNaN(h) && h > 0) {
      ...
      if (inStandard && inMini) {
        ...
        hHint.textContent = `⚠ ${h}cm - 설치 가능 범위 벗어남 (일반 92~246cm / 미니 77~230cm)`;
  ...
  buildInstallSteps(premium) {
    ...
    const kitThreshold2 = s.productType === 'mini' ? 230 : 148;
